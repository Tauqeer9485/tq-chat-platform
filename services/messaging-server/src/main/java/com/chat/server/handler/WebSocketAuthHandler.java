...
/**
 * WebSocket Session Management & JWT Validation Handler
 * 
 * Validates JWT token AFTER successful WebSocket upgrade.
 * If token is valid, creates a session and keeps connection open.
 * If token is invalid, closes the connection immediately.
 * 
 * JWT token is expected in query parameters: /ws?token=xxxxx
 */
